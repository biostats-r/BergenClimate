...
@tbl-monthly-means shows the mean monthly temperatures for Bergen for the period `r min(year(bergen$date))`--`r max(year(bergen$date))`. Mean annual temperatures in Bergen have been increasing over the study period at a rate of `r round(coef(mod)[2] * 10, 2)`°C per decade.
